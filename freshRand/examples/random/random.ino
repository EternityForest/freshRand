...
//Anything lower than 4-8 is likely to be mostly non random and highly biased, unless there is sufficent randomness in the timing and stack

//However, you might be OK using freshRand(0) in applications involving human input, because the micros() value is
//Fairly random when sampled an a random time. YMMV.

//Since this depends on temperature sensor changes, generation speed depends on the amount of noise and temperature changes.
//With an extremel stable power supply and temperature you might get several btes per minute only.
//Usually you will get more than this.
